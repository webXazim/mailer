...
async fn clean(pool: &db::DbPool) -> anyhow::Result<()> {
    ...
    sqlx::query("UPDATE account_emails SET status='failed',body='',html_body=NULL,last_error='Expired or interrupted account email',updated_at=now() WHERE (status='processing' AND updated_at<now()-interval '5 minutes') OR (status='queued' AND expires_at<now())").execute(pool).await?;
