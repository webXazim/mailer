...
- `SIGNUP_TOKEN`: generated by `production-init`; required to register a private workspace.
  Existing `.env` files need this new independent secret (`openssl rand -hex 32`).

After deployment, create your workspace using the signup token. Start in **Test**:
...
use a **Production** key for real messages. SES sandbox accounts restrict recipients;
...
The console now uses real API data and sessions. This is an **invite-only private
developer release**, not a public Resend replacement. The signup form requires
`SIGNUP_TOKEN` from `.env`; share it only with trusted testers. MFA, billing,
