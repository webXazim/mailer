...
const sections: Record<'terms' | 'privacy', Array<[string, string]>> = {
  terms: [
    ['1. Service', 'CS Mailer provides developer-facing infrastructure for submitting transactional email, configuring sending domains, managing API credentials, observing message activity, receiving webhook events, and managing recipient suppressions. Features available to a workspace may depend on its deployed configuration.'],
  ...
  privacy: [
    ...
    ['2. Account information', 'CS Mailer processes account details such as name and email address so users can authenticate, access a workspace, and receive account-related messages such as verification or password-reset instructions when those features are enabled.'],
    ['3. Developer and workspace data', 'The service processes workspace settings, API-key metadata, sending-domain configuration, DNS verification status, webhook endpoint configuration, suppression entries, and operational usage counters needed to provide the console and enforce workspace controls. Secret credential values are intended to be shown only when created or rotated.'],
    ['4. Email submission data', 'When you submit an email, the service may process sender and recipient addresses, subject, message content, metadata, attachments, delivery provider information, status, errors, and delivery events. Content availability may be limited by the service retention configuration.'],
    ['5. Delivery and webhook records', 'Operational records may include recipient delivery state, provider attempts, message identifiers, webhook delivery attempts, response status codes, retry timing, and error information. These records support troubleshooting and delivery visibility.'],
    ...
    ['8. External systems', 'Email necessarily interacts with DNS infrastructure, recipient mail servers, and configured transport systems. Webhook events are delivered to endpoints selected by workspace administrators. Information sent to those systems is also subject to their own handling practices.'],
