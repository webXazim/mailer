...
for name in CLOUDFLARE_TUNNEL_TOKEN API_AWS_ACCESS_KEY_ID API_AWS_SECRET_ACCESS_KEY WORKER_AWS_ACCESS_KEY_ID WORKER_AWS_SECRET_ACCESS_KEY OBJECT_STORAGE_ACCESS_KEY_ID OBJECT_STORAGE_SECRET_ACCESS_KEY; do
    sed -i "s/^$name=\$/$name=unused-test-credential/" "$temporary/test.env"
done
cat >>"$temporary/test.env" <<'EOF'
SES_EVENTS_QUEUE_URL=https://sqs.me-south-1.amazonaws.com/000000000000/unused
